...
1. **Add or remove exceptions** – define new non‑working periods or delete existing ones.  
2. **Specify weekdays** – control which days of the week are affected by an exception.  
3. **Handle occurrences** – manage recurring exceptions such as holidays that repeat annually.  
4. **Retrieve exceptions** – read back the defined exceptions from an existing MS Project file.

Each of these topics is covered in depth in the subsequent tutorial links, so you can jump straight to the part you need.
...
- **Performance:** Batch multiple exception updates in a single transaction to reduce file I/O overhead.
